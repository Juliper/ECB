...
public class CodeGenerator extends AstNodeBaseVisitor<Instruction, Void> {
	...
	public Instruction visitVariableAssignment(VariableAssignment variableAssignment, Void __) {
		// TODO implement (task 3.4) DOES NOT WORK
	...
	public Instruction visitMatrixLhsIdentifier(MatrixLhsIdentifier matrixLhsIdentifier, Void __) {
		// TODO implement (task 3.4)
		MatrixType matrix = ((MatrixType) matrixLhsIdentifier.getDeclaration().getType());
		visit( matrixLhsIdentifier.rowIndexExpression );
		assembler.emitBoundsCheck( 0, matrix.rows );
		int wordSizeRow = matrixLhsIdentifier.rowIndexExpression.getType().wordSize;
		int wordSizeCol = matrixLhsIdentifier.colIndexExpression.getType().wordSize;
		assembler.loadIntegerValue( wordSizeCol*matrix.cols );
		assembler.emitIntegerMultiplication();
		visit( matrixLhsIdentifier.colIndexExpression );
		assembler.emitBoundsCheck( 0, matrix.cols );

		assembler.loadIntegerValue( wordSizeCol );
		assembler.emitIntegerMultiplication();
		assembler.emitIntegerAddition();
		int offsetMatrix = matrixLhsIdentifier.getDeclaration().getLocalBaseOffset();
		assembler.loadAddress( Register.LB, offsetMatrix );
		assembler.emitIntegerAddition();
		return null;
	}
	...
	public Instruction visitVectorLhsIdentifier(VectorLhsIdentifier vectorLhsIdentifier, Void __) {
		...
		VectorType vector = ((VectorType) vectorLhsIdentifier.getDeclaration().getType());
		assembler.emitBoundsCheck( 0, vector.dimension );
		int wordSize = vectorLhsIdentifier.indexExpression.getType().wordSize;
		assembler.loadIntegerValue( wordSize );
		assembler.emitIntegerMultiplication();
		int offsetVector = vectorLhsIdentifier.getDeclaration().getLocalBaseOffset();
		assembler.loadAddress( Register.LB, offsetVector );
		assembler.emitIntegerAddition();
		return null;
	}
	...
	public Instruction visitRecordLhsIdentifier(RecordLhsIdentifier recordLhsIdentifier, Void __) {
		// TODO implement (task 3.4)
		int offsetRecord = recordLhsIdentifier.getDeclaration().getLocalBaseOffset();
		RecordType record = (RecordType) recordLhsIdentifier.getDeclaration().getType();
		int wordSizeSum = 0;
		boolean found = false;
		for(RecordElementDeclaration el : record.typeDeclaration.elements)
		{
			if(recordLhsIdentifier.elementName.equals( el.name ))
			{
				found = true;
				break;
			}
	...
	public Instruction visitDivision(Division division, Void __) {
		visitArithmeticOperator( division, true, false, false, Primitive.divI, Primitive.divF );
		return null;
	}
	...
	public Instruction visitCompare(Compare compare, Void __) {
		...
		if(compare.leftOperand.getType().equals( IntType.instance ) && compare.rightOperand.getType().equals( IntType.instance ))
...
//		if(!(matrixTranspose.operand.getType() instanceof MatrixType))
//			throw new InternalCompilerError("Matrix transpose requires matrix!");

		MatrixType matrix = (MatrixType) matrixTranspose.operand.getType();
		assembler.loadIntegerValue( matrix.rows );
